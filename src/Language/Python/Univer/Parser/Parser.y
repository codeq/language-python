...
-- Portability : ghc
--
-- Implementation of the Python version 2.x and 3.x parser.
-- Generated by happy.
-----------------------------------------------------------------------------

module Language.Python.Univer.Parser.Parser (
  parseFileInput,
  parseSingleInput,
  parseEval
) where

import Language.Python.Univer.Parser.Lexer
...
file_input : many0(either('NEWLINE', stmt))  { Module (concat (rights $1)) }

-- eval_input: testlist NEWLINE* ENDMARKER

eval_input :: { ExprSpan }
eval_input : testlist many0('NEWLINE')  { $1 }
...
decorator
  : '@' dotted_name optlist(paren_arglist) 'NEWLINE'  { makeDecorator $1 $2 $3 }
...
decorated : decorators or(classdef, funcdef)  { makeDecorated $1 $2 }

-- funcdef: 'def' NAME parameters ['->' test] ':' suite

funcdef :: { StatementSpan }
funcdef
  : 'def' NAME parameters opt(right('->', test)) ':' suite  { makeFun $1 $2 $3 $4 $6 }
...
expr_stmt
  : testlist_star_expr either(many_assign, augassign_yield_or_test_list)  { makeAssignmentOrExpr $1 $2 }
...
import_from : 'from' import_module 'import' star_or_as_names  { FromImport $2 $4 (spanning $1 $4) }
...
import_as_name : NAME optional_as_name  { FromItem $1 $2 (spanning $1 $2) }

-- dotted_as_name: dotted_name ['as' NAME]

dotted_as_name :: { ImportItemSpan }
dotted_as_name : dotted_name optional_as_name  { ImportItem $1 $2 (spanning $1 $2) }
...
assert_stmt : 'assert' sepBy(test, ',')  { AST.Assert $2 (spanning $1 $2) }
...
suite
  : simple_stmt                    { $1 }
  | 'indent' many1(stmt) 'dedent'  { concat $2 }
...
subscript
  ...
  | opt(test) ':' opt(test) opt(sliceop)  { SubscriptSlice $1 $3 $4 (spanning (spanning (spanning $1 $2) $3) $4) }
...
argument
  ...
  | test comp_for  { let span = spanning $1 $1 in ArgExpr (Generator (makeComprehension $1 $2) span) span }
...
comp_for : 'for' exprlist 'in' or_test opt(comp_iter)  { AST.CompFor $2 $4 $5 (spanning (spanning $1 $4) $5) }
